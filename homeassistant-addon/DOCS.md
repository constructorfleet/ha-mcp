...
is the only tool that sends query text to a third party.
...
### System
- `ha_config_get_yaml` **(beta — dev channel only)** — Get the current YAML fragment under a key, from one config file or across a glob.
